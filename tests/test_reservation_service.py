...
def test_confirm_returns_receipt_after_confirmation():
    service = build_service()
    session_id = "s3"
    service.state_store.save(
        session_id,
    ...
    assert response.reservation.status.value == "confirmed"
    assert response.receipt.session_id == session_id
    assert response.receipt.subtotal == 300.0
    assert response.receipt.total == 336.0
